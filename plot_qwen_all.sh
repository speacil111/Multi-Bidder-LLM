...
RESULT_ROOT="./batch_results_noqwen_mind"

mkdir -p "${PLOT_DIR}"
export MPLCONFIGDIR="/tmp/matplotlib_ds_plot"
mkdir -p "${MPLCONFIGDIR}"

readarray -t base_dirs < <(find "${RESULT_ROOT}" -maxdepth 1 -mindepth 1 -type d -name 'Qwen*' | sort)

if (( ${#base_dirs[@]} == 0 )); then
  echo "[ERROR] No Qwen result directories found under ${RESULT_ROOT}" >&2
  exit 1
fi
