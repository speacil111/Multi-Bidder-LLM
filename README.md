...
--- 
3-bidder 实验有结果后,运行下面的lambda-test:

1. `bash launcher_qwen_lambda.sh`,参数设置基本与上面的相同,多了一个lambda-list参数也不需要修改,可能需要修改的是脚本中的`GPUS_LIST="0-7" `参数.如果可以的话请先运行`bash launcher_qwen_lambda_test.sh`测试一下脚本是否可以正常跑通,再运行正常的实验.
