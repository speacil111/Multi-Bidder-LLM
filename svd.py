...
def get_better_steering_vector(model, tokenizer, layer_idx,pos_texts):
    # 正向：强调真实、诚实、可靠的语义
    neg_texts = adv.get_branded_competitor_ads()
...
def get_top_2_components_from_matrix(X):
    """
    对矩阵 X 进行中心化并进行 SVD 分解，取前两个主成分
    X: [n_total_samples, hidden_dim]
    return: vec1, vec2 (均为 [1, hidden_dim], 已归一化)
    """
    # 1. 中心化 (非常重要!)
    ...
    # 2. SVD 分解
    # full_matrices=False 比较快，我们只需要 Vh 的前几行
    try:
        U, S, Vh = torch.linalg.svd(X_centered, full_matrices=False)
    except RuntimeError as e:
        print(f"SVD计算失败，可能是显存不足或数值不稳定。尝试转到CPU计算。错误: {e}")
        X_centered_cpu = X_centered.cpu().to(torch.float32)
        U, S, Vh = torch.linalg.svd(X_centered_cpu, full_matrices=False)
        Vh = Vh.to(X.device).to(X.dtype)
    ...
    # 4. 归一化并升维
    vec1 = vec1 / (vec1.norm() + 1e-6)
    vec2 = vec2 / (vec2.norm() + 1e-6)
    
    return vec1.unsqueeze(0), vec2.unsqueeze(0)

# 多层注入配置（按需修改层索引）
target_layer_idxs = range(14,25)
pos_texts_1 = adv.get_lining_ads()
pos_texts_2 = adv.get_adidas_ads()
steering_vectors_1 = {
    idx: get_better_steering_vector(model, tokenizer, idx,pos_texts_1) for idx in target_layer_idxs
}

steering_vectors_2 = {
    idx: get_better_steering_vector(model, tokenizer, idx,pos_texts_2) for idx in target_layer_idxs
}
# SVD分解
two_component_vectors = {}
for idx in target_layer_idxs:
    # 获取两个品牌的向量
    vec_lining = steering_vectors_1[idx]
    vec_adidas = steering_vectors_2[idx]
    
    # 拼接成矩阵 [2, hidden_dim]
    matrix_input = torch.cat([vec_lining, vec_adidas], dim=0)
    
    # 调用 SVD (现在只传一个参数，正确了)
    pc1, pc2 = get_top_2_components_from_matrix(matrix_input)
    
    two_component_vectors[idx] = (pc1, pc2)
    
    
Coeff_1 , Coeff_2 = 0.8,0.2
injection_vectors = {
    idx: Coeff_1 * two_component_vectors[idx][0] + Coeff_2 * two_component_vectors[idx][1] for idx in target_layer_idxs
}
